...
class GUIListener(private val plugin: EcoPlugin) : Listener {
    ...
    fun handleClose(event: InventoryCloseEvent) {
        ...
        plugin.scheduler.run(event.inventory.location ?: event.player.location) { MenuHandler.unregisterInventory(event.inventory) }
    }
    ...
    private fun Player.renderActiveMenu() {
        ...
        rendered.render()
        plugin.scheduler.run(this) { rendered.render() }
    }
}
